...
class HybridAnalysisResult:
    ...
    llm_narrative: str = ""
    # Which model wrote the narrative, and whether it was written at all. Both
    # are surfaced to the client: without them, "no provider answered" and "the
    # last-resort model answered" are indistinguishable in the UI.
    llm_model: str = "none"
    llm_ok: bool = False
...
def run_hybrid_analysis(
    ...
    try:
        ...
        llm_narrative, llm_model, llm_ok = narrative.text, narrative.model, narrative.ok
        # Report what happened, not that the step ran. This used to say
        # "LLM narrative generated" even when every provider had refused,
        # because the failure arrived as an ordinary return value.
        if llm_ok:
            reasoning_chain.append(f"  → LLM narrative generated by {llm_model}")
        else:
            reasoning_chain.append(
                "  ⚠️ No LLM provider answered — narrative omitted, deterministic layers stand"
            )
    except Exception as e:
    ...
    analysis_breakdown = {
        ...
        "LLM Synthesis": (
            f"Narrative generated by {llm_model}, synthesizing all layer context."
            if llm_ok else
            "Unavailable — no configured provider answered. Every other layer is unaffected."
        ),
    }
